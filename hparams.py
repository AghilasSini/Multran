...
hparams = tf.contrib.training.HParams(
	...
	num_freq = 513, #1025, # (= n_fft / 2 + 1) only used when adding linear spectrograms post processing network
	...
	#Mel spectrogram
	n_fft = 1024, #2048, #Extra window size is filled with 0 paddings to match this parameter
	...
	#Tacotron
	outputs_per_step = 2,# 1 #number of frames to generate at each decoding step (increase to speed up computation and allows for higher batch size, decreases G&L audio quality)
	...
	tacotron_batch_size = 32, #32 #number of training samples on each training steps
	...
	tacotron_test_size = 0.10, #0.05, #% of data to keep as test data, if None, tacotron_test_batches must be not None. (5% is enough to have a good idea about overfit)
	...
	#Transcription into phonemes
	tacotron_phoneme_transcription = True, # Whether to transcribe words into phonemes
	path_to_espeak="/vrac/aperquin/speech/bin/espeak",
	#path_to_espeak="/home/aperquin/Programmes/speech/bin/espeak",

	#Use a reference wafeworm
	tacotron_reference_waveform = True,
	tacotron_multi_speaker = True,
	tacotron_speaker_embedding_dim = 64,
	tacotron_multi_accent = False,
	tacotron_accent_embedding_dim = 64,
	###########################################################################################################################################
