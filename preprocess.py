...
from datasets import preprocessor, preprocessor_VCTK, preprocessor_SynPaFlex
...
def preprocess(args, input_folders, out_dir, hparams):
	...
	if args.dataset.startswith("VCTK"):
		metadata, speaker_dict = preprocessor_VCTK.build_from_path(hparams, input_folders, mel_dir, linear_dir, wav_dir, args.n_jobs, tqdm=tqdm)
		pickle.dump(speaker_dict, open(os.path.join(out_dir, "speaker_dict.pkl"), "wb"))
	elif args.dataset.startswith("SynPaFlex"):
		metadata, speaker_dict = preprocessor_SynPaFlex.build_from_path(hparams, input_folders, mel_dir, linear_dir, wav_dir, args.n_jobs, tqdm=tqdm)
		pickle.dump(speaker_dict, open(os.path.join(out_dir, "speaker_dict.pkl"), "wb"))
...
def norm_data(args):
	...
	supported_datasets = ['LJSpeech-1.0', 'LJSpeech-1.1', 'M-AILABS', "SynPaFlex",'VCTK', "VCTK-Corpus_test", "VCTK-Corpus_train_dev"]
