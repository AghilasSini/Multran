...
from tacotron.utils.IPA import phoneme_described_as_categories_dict, size as nb_categories
...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None,
                   ...
                   split_infos=None, mel_references=None, speaker_id_target=None, nb_speaker=None, nb_sample=None, synthesize=False):
        """
        ...
        with tf.device(split_device):
            ...
            if self._hparams.tacotron_multi_speaker:
                tower_speaker_id_target = tf.split(speaker_id_target, num_or_size_splits=hp.tacotron_num_gpus, axis=0) if speaker_id_target is not None else speaker_id_target
            ...
            for i in range(hp.tacotron_num_gpus):
                if not self._hparams.tacotron_phoneme_transcription:
                    ...
                    tower_inputs.append(tf.reshape(p_inputs[i], [batch_size, -1, nb_categories]))
            ...
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                with tf.variable_scope('inference') as scope:
                    ...
                    if not self._hparams.tacotron_phoneme_transcription:
                        ...
                        embedded_inputs = tf.layers.dense(inputs=tf.cast(tower_inputs[i], dtype=tf.float32),
                    ...
                    if self._hparams.tacotron_multi_speaker:
                        with tf.variable_scope('speaker_encoder'):
                            conv = tf.layers.conv1d(inputs=mel_references, filters=512, kernel_size=(3,), activation=None,
                                                    padding='same')
                            conv = tf.layers.conv1d(inputs=conv, filters=512, kernel_size=(3,), activation=None,
                                                    padding='same')
                            with tf.variable_scope('biLSTM1'):
                                fw_LSTM = tf.nn.rnn_cell.BasicLSTMCell(num_units=256)
                                bw_LSTM = tf.nn.rnn_cell.BasicLSTMCell(num_units=256)
                                outputs_biLSTM, states_biLSTM = tf.nn.bidirectional_dynamic_rnn(
                                    cell_fw=fw_LSTM, cell_bw=bw_LSTM, inputs=conv, dtype=tf.float32)
                                outputs_biLSTM = tf.concat(outputs_biLSTM, axis=2)
                            with tf.variable_scope('biLSTM2'):
                                fw_LSTM = tf.nn.rnn_cell.BasicLSTMCell(num_units=256)
                                bw_LSTM = tf.nn.rnn_cell.BasicLSTMCell(num_units=256)
                                outputs_biLSTM, states_biLSTM = tf.nn.bidirectional_dynamic_rnn(
                                    cell_fw=fw_LSTM, cell_bw=bw_LSTM, inputs=outputs_biLSTM, dtype=tf.float32)
                                outputs_biLSTM = tf.concat(outputs_biLSTM, axis=2)
                            mean_over_time = tf.reduce_mean(input_tensor=outputs_biLSTM,
                                                            axis=1)  # shape (batch_size, speaker_emb_dim)
                            speaker_embedding = tf.layers.dense(inputs=mean_over_time, units=self._hparams.tacotron_speaker_embedding_dim, name="speaker_embedding")
                            repeat = tf.tile(speaker_embedding,
                                             [1, enc_tensor_shape[1]])  # shape (batch_size, speaker_emb_dim*text_len)
                            repeat = tf.reshape(repeat, (enc_tensor_shape[0], enc_tensor_shape[1],
                                                         self._hparams.tacotron_speaker_embedding_dim))  # shape (batch_size, text_len, speaker_emb_dim)
                        ...
                        attention_inputs = tf.concat([encoder_outputs, repeat], axis=-1)

                        if not synthesize:
                            with tf.variable_scope('speaker_classifier'):
                                speaker_prediction = tf.layers.dense(inputs=speaker_embedding, units=256)
                                speaker_prediction = tf.layers.dense(inputs=speaker_prediction, units=nb_speaker) # Softmax inside the loss
                    else:
                    ...
                    # Attention Mechanism
                    attention_mechanism = LocationSensitiveAttention(hp.attention_dim, attention_inputs, hparams=hp,
    ...
    def add_loss(self):
        ...
        for i in range(hp.tacotron_num_gpus):
            with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
                with tf.variable_scope('loss') as scope:
                    ...
                    # Compute speaker identity loss
                    if self._hparams.tacotron_multi_speaker:
                        speaker_identity_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=self.tower_speaker_id_target, logits=self.tower_speaker_id_pred))
                    ...
                    if self._hparams.tacotron_multi_speaker:
                        self.tower_speaker_loss.append(speaker_identity_loss)

                    loss = before + after + stop_token_loss + regularization + linear_loss
                    if self._hparams.tacotron_multi_speaker:
                        loss += speaker_identity_loss
        ...
        for i in range(hp.tacotron_num_gpus):
            ...
            if self._hparams.tacotron_multi_speaker:
                total_speaker_loss += self.tower_speaker_loss[i]
        ...
        if self._hparams.tacotron_multi_speaker:
            self.speaker_loss = total_speaker_loss / hp.tacotron_num_gpus
