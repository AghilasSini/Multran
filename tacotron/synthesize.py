...
def run_eval(args, checkpoint_path, output_dir, hparams, sentences):
	...
	with open(os.path.join(eval_dir, 'map.txt'), 'w') as file:
		for i, texts in enumerate(tqdm(sentences)):
			...
			mel_filenames, speaker_ids = synth.synthesize(texts, basenames, eval_dir, log_dir, None, mel_reference_filename)
...
def run_synthesis(args, checkpoint_path, output_dir, hparams):
	...
	with open(os.path.join(synth_dir, 'map.txt'), 'w') as file:
		for i, meta in enumerate(tqdm(metadata)):
			...
			if hparams.tacotron_reference_waveform:
				mel_reference_filename = ["/home/aperquin/Programmes/Tacotron-2-from-reference/training_data/mels/mel-p225_006.npy"]
			else:
				mel_reference_filename = None

			mel_output_filenames, speaker_ids = synth.synthesize(texts, basenames, synth_dir, None, mel_filenames, mel_reference_filename)
...
def tacotron_synthesize(args, hparams, checkpoint, sentences=None):
	...
	try:
		...
		raise RuntimeError('Failed to load checkpoint_backup at {}'.format(checkpoint))
