...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron'):
		...
		with tf.variable_scope('Tacotron_model') as scope:
			...
			initialize_args = {"inputs": inputs, "input_lengths": input_lengths, "split_infos": split_infos, "synthesize": True}
	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, mel_reference_filenames=None):
		...
		if not self._hparams.tacotron_phoneme_transcription:
			...
			seqs = [np.asarray(ipa_to_articulatory_sequence(text), dtype=np.int32) for text in texts]
		...
		if mel_reference_filenames is not None:
			np_references = [np.load(mel_reference_filename) for mel_reference_filename in mel_reference_filenames]

			# pad references according to each GPU max length
			reference_seqs = None
			for i in range(self._hparams.tacotron_num_gpus):
				device_reference = np_references[size_per_device * i: size_per_device * (i + 1)]
				device_reference, max_reference_len = self._prepare_targets(device_reference, self._hparams.outputs_per_step)
				reference_seqs = np.concatenate((reference_seqs, device_reference),
