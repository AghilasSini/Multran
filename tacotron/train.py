...
def add_train_stats(model, hparams):
    with tf.variable_scope('stats') as scope:
        ...
        if hparams.tacotron_multi_speaker:
            tf.summary.scalar('speaker_loss', model.speaker_loss)
        return tf.summary.merge_all()


def add_eval_stats(summary_writer, step, linear_loss, before_loss, after_loss, stop_token_loss, loss, speaker_loss=None):
    values = [
    ...
    if speaker_loss is not None:
        values.append(tf.Summary.Value(tag='Tacotron_eval_model/eval_stats/eval_speaker_loss', simple_value=speaker_loss))
...
def model_test_mode(args, feeder, hparams, global_step):
    with tf.variable_scope('Tacotron_model', reuse=tf.AUTO_REUSE) as scope:
        ...
        initialize_args = {"inputs": feeder.eval_inputs, "input_lengths": feeder.eval_input_lengths,
                           "mel_targets": feeder.eval_mel_targets, "stop_token_targets": feeder.eval_token_targets,
        ...
        if hparams.predict_linear:
            initialize_args["linear_targets"] = feeder.eval_linear_targets
        if hparams.tacotron_reference_waveform:
            initialize_args["mel_references"] = feeder.eval_mel_references
        if hparams.tacotron_multi_speaker:
            initialize_args["speaker_id_target"] = feeder.eval_speaker_id_target
            initialize_args["nb_speaker"] = feeder._nb_speaker
        ...
        try:
            ...
            while not coord.should_stop() and step < args.tacotron_train_steps:
                ...
                if step % args.eval_interval == 0:
                    ...
                    if hparams.tacotron_multi_speaker:
                        speaker_loss = sum(speaker_losses) / len(speaker_losses)
                    # if hparams.predict_linear:
                    #     for i in tqdm(range(feeder.test_steps)):
